...
for (bas=0;bas<taille;bas++){
	...
	for (droite=0;droite<taille;droite++){
		...
		if ((bas!=0)&&(ptsVerif[bas]==1)){
                        printf("On met tabFinal[%d][%d] à la valeur d'avant qui est %d CAR ptsVerif[%d]==1\n",bas,droite,tabFinal[bas-1][droite],bas);
			tabFinal[bas][droite]=tabFinal[bas-1][droite];
			droite++;
		}
		else {
			printf("Prmier tour ou ptsVerif[%d]==0",bas);
	}
//		for (i=0;i<taille;i++){

			printf("\n On rentre dans le for avec bas=%d si %d <= %d et si ptsVerif[%d]==%d \n\n ",bas,mat->tab[bas][droite]+tabFinal[bas-1][droite],tabFinal[bas-1][droite],bas,ptsVerif[bas]);
			if ((bas!=0)&&((mat->tab[bas][droite]+tabFinal[bas-1][droite]<=tabFinal[bas-1][droite])||(tabFinal[bas-1][droite]==9999999))&&(ptsVerif[bas]==0)){
                                printf("On met tabFinal[%d][%d] à %d\n",bas,droite,mat->tab[bas][droite]+tabFinal[bas-1][droite]);
				tabFinal[bas][droite]=mat->tab[bas][droite]+tabFinal[bas-1][i];
